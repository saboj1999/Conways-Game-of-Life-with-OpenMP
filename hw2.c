...
void ghostFill(double ***a, int N, int M)
{
    ...
    #pragma omp parallel for shared(a, N, M) private(i)
    for (i = 1; i < N - 1; i++) 
    {
        (*a)[i][M - 1] = (*a)[i][1];
        (*a)[i][0] = (*a)[i][M - 2];
    ...
    #pragma omp parallel for shared(a, N, M) private(j)
    for (j = 0; j < M; j++) 
    {
        (*a)[N - 1][j] = (*a)[1][j];
        (*a)[0][j] = (*a)[N - 2][j];
...
bool step(double **current, double ***previous, int N, int M)
{
    ...
    #pragma omp parallel for shared(current, previous, N, M) private(i, j, isCell) reduction(||:isAlive)
    for (i = 1; i < N - 1; i++) {
        for (j = 1; j < M - 1; j++) {
...
int main(int argc, char **argv) 
{
    ...
    if (argc != 5) {
      printf("Usage: %s <X-Dimension> <Y-Dimension> <Generations> <Thread-Count>\n", argv[0]);
      exit(-1);
    }
